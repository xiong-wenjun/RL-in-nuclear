...
function Pellet(
    ...
    # coordinates of the pellet 弹丸坐标
    # first point 初始点
    ...
    starting_position = [X1, Y1, Z1] #初始点
    dist = sqrt((X2 - X1)^2 + (Y2 - Y1)^2 + (Z2 - Z1)^2)
    velocity_vector = [X2 - X1, Y2 - Y1, Z2 - Z1] .* pelt.velocity_initial ./ (dist) #速度方向（单位向量）

    # trajectory for all time steps
    x, y, z = pellet_position(starting_position, velocity_vector, time, time[1]) #直线注入轨迹（似乎保持匀速）
    r, z = PAM.projected_coordinate(x, y, z) #坐标转换(x,y,z)->(r,z)

    # rho interpolant
    _, _, RHO_interpolant = IMAS.ρ_interpolant(eqt) #rho插值（归一化磁通量）

    # check the pellet position
    rho_start = RHO_interpolant.(R1, Z1) #弹丸初始位置角度rho

    if (rho_start < 1)
        error("Pellet starting inside plasma at rho = $rho_start") #如果rho<1，说明设置的弹丸初始位置就在等离子体边界内，初始位置不合理
    end

    #Bt = abs(eqt.eqt.global_quantities.magnetic_axis.r .b_field_tor) .* eqt.global_quantities.magnetic_axis.r ./r
    Bt = abs(eqt.global_quantities.vacuum_toroidal_field.b0) .* eqt.global_quantities.vacuum_toroidal_field.r0 ./ r #看公式似乎是求解每个r半径位置的磁感应强度B

    # get plasma profiles in rho coordinates and set to zero outside of the plasma
    ρ = RHO_interpolant.(r, z) #求解每个r,z位置的归一化磁通量rho
    ne = IMAS.interp1d(cp1d.grid.rho_tor_norm, cp1d.electrons.density).(ρ) #求解电子数密度ne
    ne[ρ.>1.0] .= 0.0
    Te = IMAS.interp1d(cp1d.grid.rho_tor_norm, cp1d.electrons.temperature).(ρ) #求解电子温度Te
    Te[ρ.>1.0] .= 0.0

    radii = cumsum([layer.thickness for layer in pelt.layer]) #弹丸半径
    @assert pelt.shape.size[1] == radii[end] "The layer's thickness don't add up to the total radius"
    radius = fill(0.0, size(time)) #存储弹丸半径变化
    radius[1] = radii[end] # Set initial radius at t=0
    ablation_rate = fill(0.0, size(time)) #存储烧蚀率变化
    temp_drop = fill(0.0, size(time)) #存储温度变化
    R_drift = fill(0.0, size(time)) #存储漂移
...
Return the layer index based on current pellet radius and thickness of the pellet radii
"""
# 返回当前弹丸的层索引（若弹丸由多层材料包裹而成）
...
function ablate!(eqt::IMAS.equilibrium__time_slice, cp1d::IMAS.core_profiles__profiles_1d, pelt::Pellet, surfaces::Vector{IMAS.FluxSurface})
    ...
    for k in 2:length(pelt.time)
        ...
        if pelt.ρ[k] > 1.0 && pelt.radius[k-1] > 0 # rho大于1且半径>0说明弹丸还没进入核心等离子体
            ...
            # 这里假设压强保持不变（均衡状态），因此弹丸的注入带来密度的上升，从而导致温度的下降 --> p = 2*ne*Te
            ne0 = pelt.ne[k]
